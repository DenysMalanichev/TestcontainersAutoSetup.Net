...
namespace TestcontainersAutoSetup.Core.Migrators.MigrationManagers;
...
public class EFCoreManager
{
    ...
    public async Task InitializeDatabaseAsync<TContext>(
        IContainer container, DatabaseConnection connection)
    ...
    private async Task InitializeDatabaseWithDumpAsync(IContainer container, DatabaseConnection connection)
    {
        // var latestSnapshotFile = await GetLatestSnapshotFileAsync(container).ConfigureAwait(false);
